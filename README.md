...
[![dbdev](https://img.shields.io/badge/dbdev-JesseVent%2Fsupa__aisql-3ecf8e)](https://database.dev/JesseVent/supa_aisql)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Supabase](https://img.shields.io/badge/Supabase-Edge%20Functions-3ecf8e?logo=supabase)](https://supabase.com)

`ai_sql` adds AI inference functions to your Postgres database. There's no CLI step to wire up infrastructure: a single `ai.deploy()` call bootstraps its own Supabase Edge Function straight from SQL.
...
- [Providers](#providers)
- [Aggregate safety caps](#aggregate-safety-caps)
- [Security model](#security-model)
- [Re-deploying](#re-deploying)
...
- 🧠 **LLM inference in plain SQL** — no app server, no glue code.
- 🚀 **Self-deploying** — `ai.deploy()` builds and ships its own Edge Function via the Management API.
- 🔀 **Provider-flexible** — OpenAI by default; works with any OpenAI-compatible API (Groq, Together, Azure, Ollama), or Supabase's free built-in inference.
- 📊 **Aggregate-aware** — `GROUP BY`-friendly aggregates make one LLM call per group, not per row.
- 🔒 **Secure by default** — secrets stay in Vault / Edge secrets; functions are restricted to the `authenticated` role.
...
This is the **only** secret that touches the database. It is AES-256 encrypted at rest and only ever decrypted inside `ai.deploy()` — it never appears in query logs or `pg_stat_activity`. Get your token from [supabase.com/dashboard/account/tokens](https://supabase.com/dashboard/account/tokens).

### 3. Set your provider API key

Set it via the Supabase CLI — **never pass the key as a SQL parameter**, since it would land in `pg_stat_activity` logs:
...
<details>
<summary><strong>Using a different provider?</strong> (Groq, Together, Azure, Ollama)</summary>

Any OpenAI-compatible API works. Set `OPENAI_BASE_URL` and reuse `OPENAI_API_KEY` for the credential:
...
```sql
select ai_complete('Write a one-sentence tagline for a Postgres extension.');
```
...
|---|---|---|
| `ai_complete(input, system?, model?, provider?)` | `text` | Free-form completion |
...
All scalar functions default to `provider => 'openai'`. Pass `provider => 'supabase'` for free built-in inference (see [Providers](#providers)). Aggregates always use the `openai` provider.
...
select ai_complete('Explain JSONB indexes in one sentence.');
...
```sql
-- Default provider
select ai_complete('Write a haiku about Postgres.');
...
```sql
select ai_complete('Hello', provider => 'supabase');
...
| Provider API key | Edge Function secrets (set via CLI) | Edge runtime only — never stored in DB, never a SQL parameter |
...
**Prompt injection** — caller-supplied values interpolated into system prompts (`categories`, `entity_types`, `target_language`, `schema_hint`) are sanitized (control characters stripped, length-capped) before being embedded in the prompt. The `input` field is passed as the user message — standard LLM input separation.
...
[MIT](https://opensource.org/licenses/MIT) © Jesse Vent
